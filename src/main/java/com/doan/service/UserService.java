...
public class UserService {
    ...
    public ResponseEntity<?> addUser(AddUserRequest addUserRequest) {
        ...
        try{
            ...
            return new ResponseEntity<>(new AddUserRespon(Constants.SUCCCES_CODE, Constants.MESSAGE_SUCCES_ADD, user.getUserId()), HttpStatus.OK);
        } catch (Exception e) {
    ...
    public ResponseEntity<?> getUserById(Long id) {
        try {
            ...
            if (user.isPresent()
                ...
                return new ResponseEntity<>(new GetUserByIdRespon(Constants.SUCCCES_CODE, Constants.MESSAGE_GET_USER_BY_ID_SUCCES, user.get()), HttpStatus.OK);
            } else {
                return new ResponseEntity<>(new ResponseSucces(Constants.ERR_CODE, Constants.MESSAGE_GET_USER_BY_ID_ERR), HttpStatus.INTERNAL_SERVER_ERROR);
            }
        } catch (Exception e) {
            return new ResponseEntity<>(new ResponErr(Constants.ERR_CODE, Constants.MESSAGE_GET_USER_BY_ID_ERR), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    @Transactional
